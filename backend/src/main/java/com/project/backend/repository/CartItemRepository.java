...
public interface CartItemRepository extends JpaRepository<CartItem, Long> {
    ...
    @Query("""
        ...
        AND (:userEmail IS NULL OR ci.cart.user.email LIKE %:userEmail%)
        AND (:userName IS NULL OR ci.cart.user.name LIKE %:userName%)
        AND (:productName IS NULL OR ci.variant.product.name LIKE %:productName%)
